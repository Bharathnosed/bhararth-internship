...
# Task 5 Table
cursor.execute("CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, content TEXT NOT NULL, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users (id));")

# Task 6 Tables
cursor.execute("CREATE TABLE IF NOT EXISTS questions (id INTEGER PRIMARY KEY AUTOINCREMENT, question_text TEXT NOT NULL, option_a TEXT NOT NULL, option_b TEXT NOT NULL, option_c TEXT NOT NULL, option_d TEXT NOT NULL, correct_option TEXT NOT NULL);")
cursor.execute("CREATE TABLE IF NOT EXISTS quiz_scores (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, score INTEGER NOT NULL, total INTEGER NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users (id));")

# Inject dummy questions
cursor.execute("SELECT COUNT(*) FROM questions")
if cursor.fetchone()[0] == 0:
    cursor.executemany("INSERT INTO questions (question_text, option_a, option_b, option_c, option_d, correct_option) VALUES (?, ?, ?, ?, ?, ?)", [
        ("What does HTML stand for?", "Hyper Text Markup Language", "High Text Markup Language", "Hyper Tabular Markup Language", "None of these", "A"),
        ("Which protocol is used to secure web traffic?", "HTTP", "FTP", "HTTPS", "SMTP", "C"),
        ("What does SQL stand for?", "Strong Question Language", "Structured Query Language", "Structured Question Language", "None of the above", "B")
    ])
# Inject dummy products
cursor.execute("SELECT COUNT(*) FROM products")
if cursor.fetchone()[0] == 0:
    cursor.executemany("INSERT INTO products (name, description, price, image_url) VALUES (?, ?, ?, ?)", [
        ("Wireless Headphones", "High-quality noise-canceling headphones.", 2999.00, '/static/images/1.jpg'),
        ("Mechanical Keyboard", "RGB backlit keyboard with tactile switches.", 4500.00, '/static/images/2.jpg'),
        ("Gaming Mouse", "Ergonomic 10000 DPI optical mouse.", 1200.00, '/static/images/3.jpg'),
        ("USB-C Hub", "7-in-1 multi-port adapter.", 1500.00, '/static/images/4.jpg')
    ])

conn.commit()
conn.close()
print("Database 'app.db' updated with Task 5 messaging table successfully.")